...
def process_pull_requests(repository, installation, warn_seconds=None, close_seconds=None):
    ...
    for n in pull_requests:
        ...
        if time_since_last_warning > close_seconds:
            ...
            if time_since_last_warning < 0.0:
                pr.submit_comment(
                    PULL_REQUESTS_CLOSE_WARNING.format(
                        pasttime=naturaldelta(time_since_last_commit), futuretime=naturaldelta(close_seconds)
                    )
                )
            else:
                pass
...
def main(argv=None):
    ...
    process_pull_requests(
        args.repository, installation, warn_seconds=args.warn_seconds, close_seconds=args.close_seconds
    )
