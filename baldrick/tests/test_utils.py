...
@pytest.mark.xfail(sys.platform.startswith("win"), reason="Endline comparison fails on Windows")
def test_unwrap():
    assert unwrap(WRAPPED) == UNWRAPPED


@pytest.mark.parametrize(("month", "day", "hour", "answer"), [(4, 3, 0, False), (4, 1, 0, True), (3, 31, 22, False)])
...
def test_is_special_day_3():
    ...
    assert insert_special_message(body, timestamp=t_boring, special_days=special_days) == body
    ...
    assert len(insert_special_message("", timestamp=t_boring, special_days=special_days)) == 0
    assert len(insert_special_message("", timestamp=t_special, special_days=special_days)) > 0
